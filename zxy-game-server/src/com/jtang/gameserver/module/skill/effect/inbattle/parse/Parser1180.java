 ...
 */
@Component
public class Parser1180 extends AbstractInBattleEffectParser{
...
//		addSkillReport(effect, skillRpt, context);
	...
	protected boolean castSkill(Fighter caster, Fighter target, TargetReport report, InbattleEffectConfig effect, Context context) {
		...
		FighterBuffer buffer = newBuffer(context.generateBufferId(), caster, target, AttackerAttributeKey.DODGE, effect, 1, false, BufferType.STATUS_BUFFER);
		target.addBuffer(buffer);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("[{}]释放技能[{}],对[{}]增加了闪避效果", caster.getName(), effect.getSkillName(), target.getName());
		}
		return true;
	}
	...
	protected int getParserId() {
		return InbattleParserKey.Parser1180;
	}
}
