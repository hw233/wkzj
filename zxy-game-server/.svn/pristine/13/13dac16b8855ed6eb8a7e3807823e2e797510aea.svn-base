 ...
 */
@Component
public class RecuitTaskUpdateImpl extends BaseTaskUpdate {
	@PostConstruct
	private void init() {
		eventBus.register(EventKey.RECRUIT, this);
	}

	@Override
	protected DailyTaskType getDailyTaskType() {
		return DailyTaskType.RECRUIT;
	}
	...
	public void update(GameEvent event) {
		...
		RecruitEvent recruitEvent = event.convert();
		int recruitNum = recruitEvent.num;
		updateProgress(event.actorId, task, dailyTaskVO, cfg,recruitNum);
	}
