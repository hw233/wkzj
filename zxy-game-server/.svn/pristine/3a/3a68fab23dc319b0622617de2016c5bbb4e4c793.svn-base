 ...
 */
@Component
public class Parser1221 extends AbstractInBattleEffectParser {

	@Override
	protected int getParserId() {
		return InbattleParserKey.Parser1221;
	}
	...
	protected boolean castSkill(Fighter caster, Fighter target, TargetReport report, InbattleEffectConfig effect, Context context) {
		...
		FighterBuffer buffer = newBuffer(context.generateBufferId(), caster, target, AttackerAttributeKey.IN_FIGHTING_HEAT_CHANGE, effect, ImmobilezeState.CONFUSE.getState(), true, BufferType.STATUS_BUFFER);
